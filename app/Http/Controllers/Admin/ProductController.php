...
class ProductController extends Controller
{
    ...
    public function create()
    {
        ...
        $manufactures = Manufacture::select('id','name')->where([['status', '1'],['deleted_at', NULL]])->get();
  

        $rProducts = Product::where([['status', '1'],['deleted_at', NULL]])->get();

     
        return view('admin.product.add', compact('title','parcat','rProducts','manufactures'));
    ...
    {
        $title = "Edit Product";
        $data = Product::select('products.*', 'manufactures.name AS manufacture_name')
        ...
        if (!empty($data)) {
            return view('admin.product.edit', compact('title','parcat', 'data', 'media','procategory','manufactures'));
        } else {
